...
public class ColdDown {//冷却
				...
				private static HashMap<UUID, Integer> back = new HashMap<UUID, Integer>();
				
				public static int get(Player player,TeleportType type) {//得到冷却时间
					...
					if (TeleportType.BACK.equals(type)) {
						if (!back.containsKey(uuid)) {
							return 0;
						}
				...
				public static void set(Player player,TeleportType type,int value) {//设定冷却时间
						UUID uuid = player.getUniqueId();
						if (TeleportType.HOME.equals(type)) {
							if (!home.containsKey(uuid)) {
								home.put(uuid, value);
